...
ClarityCare isn't just a hospital application — it's an engineering masterclass built to survive the most rigorous compliance audits your organisation will face. Every architectural decision was made with SOC 2 Type II, ISO 27001, HIPAA, and GDPR requirements as first-class design constraints, not afterthoughts bolted on before a deadline.

### Domain-Driven Design with Rich Aggregate Roots

The heart of ClarityCare is a pure Domain layer with **zero external dependencies**. Every business entity is a sealed aggregate root with private setters, factory methods, and state transition methods that enforce clinical workflow invariants at compile time.

No developer can accidentally set a prescription to "Dispensed" without first going through "Submitted → Approved → Dispensed". No appointment can be cancelled after completion. No patient can be archived without a documented reason. The domain layer makes illegal states unrepresentable.

**7 sealed aggregate roots** protect the most critical workflows:
- **PatientAggregate** — Registration, contact updates, archival with mandatory reason
- **AppointmentAggregate** — Booking, arrival, consultation, completion with overlap prevention
- **ConsultationAggregate** — Start, complete with mandatory summary, automatic record locking
- **PrescriptionAggregate** — Draft → Submit → Approve/Reject → Dispense (no shortcuts)
- **InvoiceAggregate** — Draft → Issue → Payment recording with balance validation
- **LabRequestAggregate** — Request → Sample → Processing → Complete (no result tampering)
- **AdmissionAggregate** — Request → Bed allocation → Admit → Discharge with summary
- **MedicationScheduleAggregate** — Active → Pause/Resume → Complete (patient safety critical)

### CQRS with MediatR — Complete Command/Query Separation

Every write operation flows through a **sealed command record** with a dedicated FluentValidation validator and a single-purpose handler. Every read operation flows through a **sealed query record** that projects directly into DTOs using `.Select()` — no entity ever loads into application memory for read operations.

```
Command Flow:  Controller → MediatR → Validator → Handler → Domain Aggregate → DbContext → Audit
Query Flow:    Controller → MediatR → Handler → .AsNoTracking().Select(DTO) → Response
```

This separation means read paths are optimised for speed (no change tracking, no navigation loading) while write paths are optimised for correctness (validation, domain invariants, audit logging).

### Bulletproof Audit Trail — SOC 2 & HIPAA Grade

Every state-changing operation in ClarityCare writes an **immutable, unalterable audit record** that captures:

| Field | Purpose | Compliance Standard |
|-------|---------|-------------------|
| UserId + UserEmail | Identity attribution | SOC 2 CC6.1, HIPAA §164.312(b) |
| IpAddress | Source identification | ISO 27001 A.12.4.1 |
| CorrelationId | Distributed request tracing | SOC 2 CC7.2 |
| EntityName + EntityId | Target identification | GDPR Art. 30 |
| OldValues + NewValues (JSON) | Complete data delta | HIPAA §164.312(b), SOC 2 CC8.1 |
| Action + Severity | Classification | ISO 27001 A.12.4.1 |
| TimestampUtc | Temporal ordering | All standards |

**Immutability is enforced architecturally:** The AuditService only performs INSERT operations. No UPDATE or DELETE method exists. No API endpoint exposes audit mutation. Even system administrators cannot alter the audit trail through any application interface.

### Distributed Tracing — End-to-End Request Correlation

Every HTTP request from the Angular frontend carries a unique `X-Correlation-Id` header generated by a dedicated interceptor. This ID propagates through:

1. **Angular Interceptor** — Generates UUID, attaches to outgoing request
2. **CorrelationId Middleware** — Preserves or generates server-side, sets TraceIdentifier
3. **CurrentUserService** — Makes ID available to all handlers and services
4. **AuditService** — Writes ID into every audit log entry
5. **Exception Middleware** — Includes ID in error responses for support correlation
6. **Response Headers** — Returns ID to frontend for client-side debugging

This means any support ticket referencing an error can be traced from the user's browser through the API pipeline into the exact database record that was affected.

### RFC 7807 Exception Shielding — Zero Information Disclosure

ClarityCare's global exception middleware implements the Internet standard RFC 7807 (Problem Details) while enforcing strict information disclosure prevention:

- **400 Bad Request** — Validation errors with field-level error dictionaries
- **401 Unauthorized** — Generic "authentication required" (no hint about what credential failed)
- **403 Forbidden** — "Access denied" (no hint about which permission is missing)
- **404 Not Found** — Safe resource-not-found message
- **409 Conflict** — Business conflict description (e.g., "clinician has overlapping appointment")
- **422 Unprocessable Entity** — Domain invariant violation (business rule broken)
- **500 Internal Server Error** — **NEVER exposes stack traces, inner exceptions, or system paths.** Returns only a 12-character ErrorId that support can use to find the full exception in internal logs.

### Zero-Trust Security Model

ClarityCare implements defence-in-depth at every layer:

**Authentication:** JWT Bearer tokens with configurable expiry, role claims, and 31 granular permission claims. PBKDF2 password hashing with 100,000 iterations and per-user salt. Constant-time comparison preventing timing attacks.

**Authorisation:** Every API endpoint requires `[Authorize]`. Controllers enforce role-based access. Handlers verify resource-level ownership before processing.

**Input Validation:** FluentValidation runs as a MediatR pipeline behavior BEFORE any handler code executes. Parameterised EF Core queries prevent SQL injection. No string concatenation in any query path.

**Transport Security:** HTTPS enforcement in production. CORS policy restricting API access to authorised Angular origin only.

### Database Engineering — Production Hardened

**Entity Framework Core** with SQL Server configured for enterprise resilience:

- **Retry Strategy** — `EnableRetryOnFailure(maxRetryCount: 3, maxRetryDelay: 10s)` handles transient network failures
- **Command Timeout** — 30 seconds prevents query hangs from blocking thread pool
- **Explicit Configurations** — Every entity has `IEntityTypeConfiguration<T>` with `HasMaxLength()`, `HasPrecision()`, explicit FK relationships, and named indexes
- **Performance Indexes** — Composite indexes on all high-query-volume columns (patient search, appointment scheduling, audit lookups)
- **Concurrency Control** — `RowVersion` optimistic locking on critical resources (beds, admissions) preventing race conditions
- **Soft Delete Architecture** — `AuditableEntity` base class with `IsDeleted`, `DeletedAtUtc`, `DeletedBy` — data is never physically destroyed

### Angular Enterprise Frontend

The Angular 20 frontend demonstrates modern SPA architecture:

- **Design System** — 30+ CSS component classes (`cc-btn-primary`, `cc-card`, `cc-table`, `cc-badge-*`) with CSS custom properties for theme consistency
- **Reusable DataTable** — Enterprise listing component with built-in search (400ms debounce), pagination, sorting, badge mapping, empty states, and loading indicators
- **API Service Layer** — Generic `ApiService` with typed `get<T>()`, `post<T>()`, `list<T>()` methods — no `HttpClient` in components
- **Interceptor Pipeline** — `Correlation → Auth → Error` — every request traced, authenticated, and error-handled
- **Permission-Based UI** — Sidebar navigation shows only authorised modules based on JWT permission claims
- **Signal-Based State** — Angular Signals for all component state with `ChangeDetectionStrategy.OnPush`
- **Standalone Components** — Zero NgModules, tree-shakeable, lazy-loaded per route

### The Architecture Senior Engineers Appreciate

```
┌─────────────────────────────────────────────────────────────────┐
│                    ANGULAR 20 SPA                                 │
│  Interceptors → Services → Components (Signals, OnPush)          │
└──────────────────────────────┬──────────────────────────────────┘
                               │ HTTPS + JWT + X-Correlation-Id
┌──────────────────────────────▼──────────────────────────────────┐
│                    ASP.NET CORE 10 API                            │
│  Correlation MW → Exception MW → Auth → Controllers (sealed)     │
├──────────────────────────────────────────────────────────────────┤
│                    APPLICATION LAYER                              │
│  MediatR Pipeline: Validation → Handler → Audit                  │
│  Sealed Commands/Queries + FluentValidation + DTO Projection     │
├──────────────────────────────────────────────────────────────────┤
│                    DOMAIN LAYER (Zero Dependencies)               │
│  Sealed Aggregates → Factory Methods → State Machines            │
│  DomainException on invariant violation → 422 to client          │
├──────────────────────────────────────────────────────────────────┤
│                    INFRASTRUCTURE LAYER                           │
│  EF Core + SQL Server (Retry + Timeout + AsNoTracking)           │
│  AuditService (Write-Only) + CurrentUserService (IP + Corr.Id)   │
└──────────────────────────────────────────────────────────────────┘
```
